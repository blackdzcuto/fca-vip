  ...
  switch (data.__typename) {
    ...
    case "UserMessage":
      return {
        ...
        attachments: data.blob_attachments && data.blob_attachments.length > 0 ? data.blob_attachments.length.map(att => {
          var x;
          try {
...
module.exports = function (defaultFuncs, api, ctx) {
  return function getMessage(threadID, messageID, callback) {
    ...
    defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, form).then(utils.parseAndCheckLogin(ctx, defaultFuncs)).then(resData => {
