...
module.exports = function (http, api, ctx) {
  function handleGetData(userIDs) {
    ...
    userIDs.map(function (c) {
      var mainPromise = utils.get("https://graph.facebook.com/" + c, ctx.jar, {
      ...
      }, ctx.globalOptions).then(function (res) {
        if (res.body.indexOf("<") < 0) {
    ...
    // resolve all promise
    Promise.all(uploads).then(function (userData) {
  ...
  return function getUserInfo(userIDs, useGraph, callback) {
    ...
    if (useGraph) {
      ...
      handleGetData(userIDs).then(function (res) {
        return cb(null, res);
      }).catch(function (err) {
        log.error("getUserInfo", err);
        return cb(err);
      ...
      http.post("https://www.facebook.com/chat/user_info/", ctx.jar, form).then(utils.parseAndCheckLogin(ctx, http)).then(function (res) {
