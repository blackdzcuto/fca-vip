...
module.exports = function (http, api, ctx) {
  function handleAvatar(userIDs, height, width) {
    ...
    userIDs.map(function (v) {
      var mainPromise = http.get(`https://graph.facebook.com/${v}/picture?height=${height}&width=${width}&redirect=false&access_token=${ctx.access_token}`, ctx.jar).then(utils.parseAndCheckLogin(ctx, http)).then(function (res) {
        return {
          userID: v,
          url: res.data.url
        };
    ...
    Promise.all(uploads).then(function (res) {
      return cb(null, res.reduce(function (Obj, {
        userID,
        url
      }) {
        Obj[userID] = url;
  ...
  return function getAvatarUser(userIDs, size = [1500, 1500], callback) {
    ...
    ;
    handleAvatar(userIDs, height, width).then(function (res) {
      return cb(null, res);
    }).catch(function (err) {
