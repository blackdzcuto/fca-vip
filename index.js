...
function buildAPI(globalOptions, html, token, jar) {
  var {
    c_user,
    i_user
  } = jar.getCookies("https://www.facebook.com").reduce(function (form, val) {
  ...
  var api = {
    ...
    getCookie: function getCookie() {
      return jar.getCookies("https://www.facebook.com/").join("; ");
    }
  };
  ...
  let irisSeqID = null;
  if (!token) {
    log.warn("login", "Cant get access_token from business.facebook.com");
  }
  if (oldFBMQTTMatch) {
  ...
  require("node:fs").readdirSync(__dirname + "/src/").filter(v => v.endsWith(".js")).map(function (v) {
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
  return function (res) {
    ...
    return utils.post("https://www.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=110", jar, form, loginOptions).then(utils.saveCookies(jar)).then(function (res) {
      ...
      if (headers.location.includes(".com/checkpoint/")) {
        ...
        if (callback == prCallback) {
          throw {
            error: "Promise is not supported for login code verification"
          };
        }
        var Referer = headers.location;
        return utils.get(Referer, jar, null, loginOptions).then(utils.saveCookies(jar)).then(function (res) {
          var html = res.body;
          ...
          if (html.includes(".com/checkpoint/?next")) {
            function submit2FA(code) {
              ...
              if (typeof code == "string") {
                utils.post(Referer, jar, form, loginOptions, null, {
                  Referer
                  ...
                  return utils.post(Referer, jar, form, loginOptions, null, {
                    Referer
            ...
            function submitNot2FA() {
              ...
              utils.post(Referer, jar, form, loginOptions).then(utils.saveCookies(jar)).then(function () {
                // Use the same form (safe I hope)
                form.name_action_selected = "save_device";
                return utils.post(Referer, jar, form, loginOptions).then(utils.saveCookies(jar));
              }).then(function (res) {
                var headers = res.headers;
      ...
      return utils.get("https://www.facebook.com/", jar, null, loginOptions, null, {
        noRef: true
      }).then(utils.saveCookies(jar));
    });
  };
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  if (appState) {
    ...
    if (typeof appState == "string") {
      appState = decodeURIComponent(appState).split(";").filter(item => item != "").map(function (c) {
    ...
    // Load the main page.
    mainPromise = utils.get("https://www.facebook.com/", jar, null, globalOptions, null, {
      noRef: true
    }).then(utils.saveCookies(jar));
  } else {
    ...
    mainPromise = utils.get("https://www.facebook.com/", null, null, globalOptions, null, {
      noRef: true
    }).then(utils.saveCookies(jar)).then(makeLogin(jar, email, password, globalOptions, callback, prCallback));
  }
  var ctx;
  var http;
  var api;
  mainPromise = mainPromise.then(function (res) {
    ...
    return res;
  }).then(utils.getAccessFromBusiness(jar, globalOptions)).then(function (res) {
