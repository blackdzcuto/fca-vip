...
var topics = ["/legacy_web", "/webrtc", "/rtc_multi", "/onevc", "/br_sr",
//Notification
//Need to publish /br_sr right after this
"/sr_res", "/t_ms", "/thread_typing", "/orca_typing_notifications", "/notify_disconnect",
//Need to publish /messenger_sync_create_queue right after this
"/orca_presence",
//Will receive /sr_res right here.
"/inbox", "/mercury", "/messaging_events", "/orca_message_notifications", "/pp", "/webrtc_response"];
function getSeqID(http, api, ctx, globalCallback) {
  ...
  http.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, form).then(utils.parseAndCheckLogin(ctx, http)).then(function (res) {
...
function markDelivery(ctx, api, threadID, messageID) {
  if (threadID && messageID && !ctx.stopListen) {
    api.markAsDelivered(threadID, messageID).then(function () {
      if (!ctx.globalOptions.autoMarkRead && ctx.stopListen) {
        return null;
      } else {
        return api.markAsRead(threadID);
      }
    }).catch(_ => {});
  }
}
function parseDelta(http, api, ctx, v, globalCallback) {
  if (v.delta.class == "NewMessage") {
    if (ctx.globalOptions.pageID != v.queue) {
      return;
    }
    (function resolveAttachmentUrl(i) {
      ...
      } else if (v.delta.attachments && v.delta.attachments[i].mercury.attach_type == "photo") {
        api.resolvePhotoUrl(v.delta.attachments[i].fbid).then(url => {
      ...
      clientPayload.deltas.map(function (delta) {
        if (delta.deltaMessageReaction && ctx.globalOptions.listenEvents) {
          ...
          if (delta.deltaMessageReply.repliedToMessage) {
            ...
            http.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, {
              av: ctx.globalOptions.pageID,
              queries: JSON.stringify({
                o0: {
            ...
            }).catch(err => log.error("forcedFetch", err)).finally(function () {
  ...
  switch (v.delta.class) {
    ...
    case "ParticipantLeftGroupThread":
      try {
        ...
        if (!ctx.globalOptions.selfListen && formattedEvent.author.toString() === ctx.userID || !ctx.loggedIn) {
          break;
        }
    ...
    class MessageEmitter extends EventEmitter {
      stopListening(stopCallback) {
        ...
        if (ctx.mqttClient) {
          ...
          ctx.mqttClient.end(false, function (...message) {
