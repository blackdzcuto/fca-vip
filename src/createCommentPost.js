...
module.exports = function (http, api, ctx) {
  function handleUpload(msg, form) {
    ...
    for (let item of msg.attachments) {
      if (!utils.isReadableStream(item)) {
        return cb({
          error: "image should be a readable stream and not " + utils.getType(image)
        });
      }
      var httpData = http.postFormData("https://www.facebook.com/ajax/ufi/upload/", ctx.jar, {
  ...
  return function createCommentPost(msg, postID, callback, replyCommentID) {
    ...
    handleUpload(msg, form).then(_ => handleURL(msg, form)).then(_ => handleMentions(msg, form)).then(_ => handleSticker(msg, form)).then(_ => createContent(form)).then(info => cb(null, info)).catch(function (err) {
