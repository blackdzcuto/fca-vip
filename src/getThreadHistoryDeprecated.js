...
module.exports = function (defaultFuncs, api, ctx) {
  return function getThreadHistory(threadID, amount, timestamp, callback) {
    ...
    if (!callback) {
      throw {
        error: "getThreadHistory: need callback"
      };
    }
    var form = {
      client: "mercury"
    ...
    api.getUserInfo(threadID, function (err, res) {
      ...
      defaultFuncs.post("https://www.facebook.com/ajax/mercury/thread_info.php", ctx.jar, form).then(utils.parseAndCheckLogin(ctx, defaultFuncs)).then(function (resData) {
        if (resData.error) {
          throw resData;
        ...
        api.getUserInfo(Object.keys(userIDs), function (err, data) {
          ...
          resData.payload.actions.forEach(function (v) {
            ...
            if (sender) {
              v.sender_name = sender.name;
            } else {
              v.sender_name = "Facebook User";
            }
