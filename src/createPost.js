...
module.exports = function (http, api, ctx) {
  function handleUpload(msg, form) {
    ...
    if (!msg.attachment) {
      ...
      for (let attachment of msg.attachment) {
        ...
        var main = http.postFormData("https://upload.facebook.com/ajax/react_composer/attachments/photo/upload", ctx.jar, vari).then(utils.parseAndCheckLogin(ctx, http)).then(function (res) {
  ...
  function handleUrl(msg, form) {
    ...
    if (!msg.url) {
      ...
      http.post("https://www.facebook.com/api/graphql/", ctx.jar, {
  ...
  function createContent(vari) {
    ...
    http.post("https://www.facebook.com/api/graphql/", ctx.jar, form).then(utils.parseAndCheckLogin(ctx, http)).then(res => cb(null, res)).catch(cb);
    return rt;
  }
  return function createPost(msg, callback) {
    ...
    handleUpload(msg, form).then(_ => handleUrl(msg, form)).then(_ => handleMention(msg, form)).then(_ => createContent(form)).then(function (res) {
      if (res.error || res.errors) {
        throw res;
      }
