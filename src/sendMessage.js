...
module.exports = function (http, api, ctx) {
  function handleUpload(msg, form) {
    ...
    if (msg.attachments.length > 0) {
      ...
      for (let item of msg.attachments) {
        ...
        var httpData = http.postFormData("https://upload.facebook.com/ajax/mercury/upload.php", ctx.jar, {
          upload_1024: item,
          voice_clip: "true"
      ...
      Promise.all(uploads).then(function (main) {
        main.forEach(item => {
  ...
  function handleURL(msg, form) {
    ...
    if (msg.url) {
      ...
      http.post("https://www.facebook.com/message_share_attachment/fromURI/", ctx.jar, {
        image_height: 960,
        image_width: 960,
        uri: msg.url
      }).then(utils.parseAndCheckLogin(ctx, http)).then(function (res) {
  ...
  function sendContent(threadID, form, isSingleUser, messageAndOTID) {
    ...
    http.post("https://www.facebook.com/messaging/send/", ctx.jar, form).then(utils.parseAndCheckLogin(ctx, http)).then(function (res) {
      form = null;
      if (!res) {
        throw {
  ...
  return function sendMessage(msg, threadID, callback, messageID, isGroup) {
    ...
    if (typeMessage == "String" || typeMessage == "Array") {
      ...
      if (msg.emoji) {
        if (utils.getType(msg.emoji) == "Object") {
          null;
        } else {
          msg.emoji = null;
        }
    ...
    handleUpload(msg, form).then(_ => handleURL(msg, form)).then(_ => {
      if (msg.sticker) {
      ...
      if (msg.emoji) {
        if (!msg.emoji.emojiSize) {
          msg.emoji.emojiSize = "medium";
        } else {
          null;
        }
    ...
    }).then(info => cb(null, info)).catch(function (err) {
